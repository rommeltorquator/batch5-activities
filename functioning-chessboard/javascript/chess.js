...
const TILE_SIZE = 100; // 50
const WHITE_TILE_COLOR = "rgb(250, 235, 215)";
const BLACK_TILE_COLOR = "rgb(85, 107, 47)";
const HIGHLIGHT_COLOR = "#9ab173";
const WHITE = 0;
const BLACK = 1;
    ...
    for (let i = 0; i < BOARD_HEIGHT; i++) {
        for (let j = 0; j < BOARD_WIDTH; j++) {
            ...
            if (board.tiles[i][j].team === WHITE) {
                chessCtx.fillStyle = "#4e1773";
            } else {
                chessCtx.fillStyle = "#000000";
            }
                        
            chessCtx.font = "58px Arial";
            let pieceType = board.tiles[i][j].pieceType;
            chessCtx.fillText(piecesCharacters[pieceType], TILE_SIZE*(j+1.8/8), TILE_SIZE*(i+3.5/5));
        }
    }
}
